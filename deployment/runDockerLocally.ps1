...
Start-Process -FilePath "docker" -ArgumentList "run --name stockripper-chat-app -v $env:userprofile\.azure:/root/.azure -p 5004:5004 --network stockripper-network --network-alias stockripper-chat-app.stockripper.internal --env-file $envFilePath -e AGENT_SERVICE_URL=http://stockripper-agent-app.stockripper.internal:5000/agents stockripper-chat-app:latest"
